...
class LlavaHFForAnalysis():
    ...
    def __init__(self, args):
        if args.load_4bit:
            ...
            quant_config = None
        self.model = LlavawithGradient.from_pretrained(
            args.model_name,
        ...
        self.model.num_img_patches = (self.model.config.vision_config.image_size // self.model.config.vision_config.patch_size)**2
    ...
    def register_hooks(self):
        ...
        for layer in self.model.vision_tower.vision_model.encoder.layers[:self.model.config.vision_feature_layer+1]:
    ...
    def chat(self, image, text):
        ...
        answer_ids = answer_ids[0, input_ids_len:]
        return answer_ids
    ...
    def forward_with_grads(self, image, text, answer):
        ...
        outputs.logits = outputs.logits[0, self.ans_start_idx-1:-1] # logits预测的是下一个token，要用于梯度回传，-1

        img_hidden_states = [h[0, self.img_start_idx:self.img_end_idx] for h in outputs.hidden_states]
        outputs.img_hidden_states = [tensor.to(torch.float32) for tensor in img_hidden_states] # 

        outputs.hidden_states = [h[0, self.ans_start_idx-1:-1] for h in outputs.hidden_states]    # hiddenstate是当前token的，由于只用于token_evo，因此也-1
        return outputs
...
class LlavawithGradient(LlavaForConditionalGeneration):
    # def generate(
    #     self,
